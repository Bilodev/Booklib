...
<home>
  ...
  {#if minimalView && bookViews.length && !foundBooks.length}
    ...
    <table class="book-table">
      <tr>
        <th class="book-table-col-number"> <center>#</center></th>
        <th>Title</th>
        <th>Author</th>
        <th class="book-table-col-status">Status</th>
        <th class="book-table-col-remove"></th>
      </tr>
  ...
  {#if foundBooks.length && !minimalView}
    ...
    <div class="book-card-container">
      {#each foundBooks as bk}
        <div class="book-card">
          <SearchResultC {bk} status={isBookInList(bk.id, bookViews)[1]}
          ></SearchResultC>
        </div>
      {/each}
    </div>
